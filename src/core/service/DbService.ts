...
export default abstract class DBService extends ExternalService {
    ...
    public async RunQuery<T>(req: Request): Promise<T> {
        ...
        this.logger.trace(`Running request against database service: ${this.serviceName}`, req);

        if (!this.serviceConnection) {
            this.logger.trace(`Service connection is undefined for service: ${this.serviceName}`);
    ...
    public async Run<T>(
        ...
        try {
            if (req) {
                this.logger.trace("Running custom query with request transaction context", query, req);
                const res = await this.serviceConnection.transaction(req).run(query);

                this.logger.trace("Custom query with request context to database handled successfully",
                    query, req);
