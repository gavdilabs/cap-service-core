...
export default abstract class ExternalService {
  ...
  public async Connect(): Promise<void> {
    ...
    try {
      this.logger.trace(
        `Establishing connection to service: ${this.serviceName}`,
      );
      this.serviceConnection = await cds.connect.to(this.serviceName);
      this.logger.trace(
        `Connection established to service: ${this.serviceName}`,
      );
    } catch (e) {
      this.logger.error(
  ...
  public GetConnection(): Service | undefined {
    this.logger.trace(
      `Fetching established connection for service: ${this.serviceName}`,
